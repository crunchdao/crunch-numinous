...
class DesearchClient:
    ...
    async def ai_search(
        ...
        async def _call() -> AISearchResponse:
        ...
        return await with_retry("desearch", _call)
    ...
    async def web_links_search(
        ...
        async def _call() -> WebLinksResponse:
            async with aiohttp.ClientSession(timeout=self.__timeout, headers=self.__headers) as session:
                async with session.post(url, json=body) as response:
                    log_rate_limit_headers("desearch", response)
                    response.raise_for_status()
    ...
    async def web_search(
        ...
        async def _call() -> WebSearchResponse:
            async with aiohttp.ClientSession(timeout=self.__timeout, headers=self.__headers) as session:
    ...
    async def web_crawl(self, url: str) -> WebCrawlResponse:
        ...
        async def _call() -> WebCrawlResponse:
    ...
    async def x_search(
        ...
        async def _call() -> list[XPostSummary]:
            async with aiohttp.ClientSession(timeout=self.__timeout, headers=self.__headers) as session:
                async with session.get(url, params=params) as response:
                    log_rate_limit_headers("desearch", response)
                    response.raise_for_status()
